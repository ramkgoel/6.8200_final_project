...
def train_model(
    ...
    for epoch in (e_iter := tqdm(range(num_epochs))):
        for batch in (b_iter := tqdm(dataloader)):
        ...
        print(losses[-1])
    return losses, vision_encoder, noise_pred_net
...
results = train_model(resnet=resnet, device=device)
filename = f"results_tuning/vision_results_best.pkl"
with open(filename, "wb") as f:
    pkl.dump(results, f)
torch.save(results, filename.replace(".pkl", ".pt"))

# DIFF TIMESTEPS

# diff_timesteps = int(sys.argv[1])
# device = f"cuda:{int(sys.argv[2])}"

# results = train_model(diffusion_timesteps=diff_timesteps, device=device)
# filename = f"results_tuning/vision_results_diffusiontimesteps_{diff_timesteps}_tuning.pkl"
# with open(filename, "wb") as f:
#     pkl.dump(results, f)
# torch.save(results, filename.replace(".pkl", ".pt"))
...
# results = train_model(obs_horizon=horizon, device=device)
# filename = f"results_tuning/vision_results_obs_horizon_{horizon}_tuning.pkl"
# with open(filename, "wb") as f:
#     pkl.dump(results, f)
# torch.save(results, filename.replace(".pkl", ".pt"))